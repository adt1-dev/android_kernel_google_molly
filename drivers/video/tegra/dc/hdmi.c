...
static int tegra_dc_hdmi_init(struct tegra_dc *dc)
{
	...
	spin_lock_init(&hdmi->state_lock);
...
static void tegra_dc_hdmi_setup_avi_infoframe(struct tegra_dc *dc, bool dvi)
{
	...
	dev_dbg(&dc->ndev->dev, "HDMI AVI vic=%d m=%d q=%d\n",
		avi.vic, avi.m, avi.q);
...
static void tegra_dc_hdmi_enable(struct tegra_dc *dc)
{
	...
	if (oldrate == dc->mode.pclk) {
		/* allowing the multipler change would cause
		 * HDMI glitch during boot if bootloader set
		 * an image already with the clk already at
		 * the desired freq.
		 */
		clk_prepare_enable(hdmi->clk);
	} else {
	 ...
	 */
	if (edid_read) {
		/* after boot, this is called called by hwc via
		 * ioctl blank/unblank, which is done after EDID
		 * has been read.
		 */
	...
	val = tegra_dc_readl(dc, DC_CMD_DISPLAY_COMMAND);
	if (val == DISP_CTRL_MODE_C_DISPLAY) {
		pr_info("%s: display already in continuous mode, skip init\n",
			__func__);
		goto skip;
	}
